...
class KernelThreshold {
 public:
  ...
  __aicore__ inline void Init(ThresholdOpencvTilingData* tiling,
    ...
    xGM.SetGlobalBuffer((__gm__ T*)x + vecTiling.blockOffset,
                        vecTiling.blockLength);
    yGM.SetGlobalBuffer((__gm__ T*)y + vecTiling.blockOffset,
                        vecTiling.blockLength);
 ...
 private:
   ...
   */
  __aicore__ inline LocalTensor<C> CastInput(
   ...
   */
  __aicore__ inline LocalTensor<C> GetOutput(
  ...
  __aicore__ inline void FreeInput(TQue<QuePosition::VECIN, BUFFER_NUM>& queue,
                                   LocalTensor<C>& xLocal) {
    if (std::is_same<T, C>::value) queue.FreeTensor(xLocal);
  }
  ...
  __aicore__ inline void CompareWrap(const LocalTensor<uint8_t>& dstLocal,
    ...
    if (tailCount != 0) {
      ...
      Compare(dstLocal[maskIdx], src0Local[tailIdx], src1Local[tailIdx],
              cmpMode, tailCount, 1, repeatParams);
    }
  }
  ...
  __aicore__ inline void SelectWrap(const LocalTensor<C>& dstLocal,
    ...
    if (tailCount != 0) {
      ...
      uint32_t tailIdx = calCount - tailCount;
      uint32_t maskIdx = tailIdx / sizeof(uint8_t);
      Select(dstLocal[tailIdx], selMask[maskIdx], src0Local[tailIdx], src1Local,
             selMode, tailCount, 1, repeatParams);
    }
  }

  __aicore__ inline void Threshold(LocalTensor<C>& xLocal,
